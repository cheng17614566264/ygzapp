				...
				<td class="centercondition"><s:component
					...
					<div id="tbl_current_status">
						<img src="<%=bopTheme%>/themes/images/icons/icon13.png" /> <span
							...
							class="current_status_submenu">复核数据</span>
					</div>
							...
							<tr>
								<td><s:if test="specialTicket.level1Option == 0">
										<input type="radio" value="0"
											...
											readonly="readonly" />已抵扣
						</s:if> <s:else>
										<input type="radio" value="0"
											name="specialTicket.level1Option" readonly="readonly" />已抵扣
						</s:else> <br> <s:if test="specialTicket.level1Option == 1">
										<input type="radio" value="0"
											name="specialTicket.level1Option" checked="checked"
											readonly="readonly" />未抵扣
						</s:if> <s:else>
										<input type="radio" value="1" id="radioF"
											name="specialTicket.level1Option" readonly="readonly" />未抵扣
						</s:else></td>
								<td colsapn="3"><s:if
										test="specialTicket.level2Option == 0">
										<input type="radio" value="0"
											name="specialTicket.level2Option" checked="checked"
											readonly="readonly" />无法认证<br />
									</s:if> <s:else>
										<input type="radio" value="0"
											name="specialTicket.level2Option" readonly="readonly" />无法认证<br />
									</s:else> <s:if test="specialTicket.level2Option == 1">
										<input type="radio" value="1"
											name="specialTicket.level2Option" checked="checked"
											readonly="readonly" />纳税人识别号认证不符<br />
									</s:if> <s:else>
										<input type="radio" value="1"
											name="specialTicket.level2Option" readonly="readonly" />纳税人识别号认证不符<br />
									</s:else> <s:if test="specialTicket.level2Option == 2">
										<input type="radio" value="2"
											name="specialTicket.level2Option" checked="checked"
											readonly="readonly" />增值税专用发票代码、号码认证不符<br />
									</s:if> <s:else>
										<input type="radio" value="2"
											name="specialTicket.level2Option" readonly="readonly" />增值税专用发票代码、号码认证不符<br />
									</s:else> <s:if test="specialTicket.level2Option == 3">
										<input type="radio" value="3"
											name="specialTicket.level2Option" checked="checked"
											readonly="readonly" />所购货物或劳务、服务不属于增值税扣税项目范围<br />
									</s:if> <s:else>
										<input type="radio" value="3"
											name="specialTicket.level2Option" readonly="readonly" />所购货物或劳务、服务不属于增值税扣税项目范围<br />
									</s:else></td>
							</tr>
							<tr>
								<td width="15%"
