				...
				<td class="centercondition">
					<div id="tbl_current_status">
						<img src="<%=bopTheme%>/themes/images/icons/icon13.png" /> <span
							...
							class="current_status_submenu">进项转出比例查看</span>
					</div> <!-- 查询条件功能框 -->
						...
						<table id="tbl_query" cellpadding="0" cellspacing="0" border="0">
							<tr>
								...
								<td colspan="4"><input type="button"
									...
									onclick="submitForm('listInvoiceInSurtaxBLSelect.action');"
					...
					<div id="lessGridList4" style="overflow: auto; width: 100%;">
						<table class="lessGrid" cellspacing="0" rules="all" border="0"
							...
							<tr class="lessGrid head">
								...
								<%    
									  List<String> list=(List)request.getAttribute("mList");
									  for(int i=list.size()-1;i>=0;i--){
										  String s = list.get(i);
										  %>
								<th style="text-align: center" colspan="3"><%= s+"月" %></th>
								<%
									  }
								%>

							</tr>
							<%   List<Proportionality> blList=(List)request.getAttribute("biglist");
							...
							<tr>
								...
								<td style="text-align: center"><%= prop.getInstName() %></td>
								<%  List<String> lStrings=prop.getResultList(); 
													if(lStrings!=null&&lStrings.size()>0){
														for(int k=0;k<lStrings.size();k++){
															%>
								<td style="text-align: center"><%= lStrings.get(k).split("-")[0] %></td>
								<td style="text-align: center"><%= lStrings.get(k).split("-")[1].equals("X")?"0.0":lStrings.get(k).split("-")[1] %></td>
								<td style="text-align: center"><%= com.cjit.vms.input.util.Util.getDatasource(lStrings.get(k).split("-")[2])%></td>
								<%
														}
													}
												%>
