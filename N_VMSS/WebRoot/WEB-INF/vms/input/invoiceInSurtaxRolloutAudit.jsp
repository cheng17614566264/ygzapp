...
<script type="text/javascript">
	...
	function rolloutAudit(val){
		...
		if("1"==val){
			//审核通过
			if(window.confirm("确定将在同一个汇总机构的下的数据执行通过操作？")){
				audit($selects.val(), "1");
			}
		}else{
			if(window.confirm("确定将在同一个汇总机构的下的数据执行拒绝操作？")){
				OpenModalWindowSubmit("InvoiceCancel.action?selects="+$selects.val(),500,250,true);
				document.forms[0].action = "listInvoiceInSurtaxRolloutAudit.action";
	...
	function checkjy(date){
		var bool = "N";
		$.ajax({
		       url: 'AuditRolloutCheck.action',
		       type: 'POST',
		       async: false,
		       data: {date: date},
		       dataType: 'text',
				...
				<td class="centercondition">
					<div id="tbl_current_status">
						<img src="<%=bopTheme%>/themes/images/icons/icon13.png" /> <span
							...
							class="current_status_submenu">转出比例调整审核</span>
					</div>
						...
						<table id="tbl_query" cellpadding="0" cellspacing="0" border="0">
							<tr>
								...
								<td colspan="4"><input type="button"
									...
									onclick="submitForm('listInvoiceInSurtaxRolloutAudit.action');"
					...
					<table id="tbl_tools" width="100%" border="0">
						<tr align="left">
							<td style="text-align: left"><a href="javascript:void();"
								onclick="submitForm('exportInvoiceRolloutAudit.action')"> <img
									src="<c:out value="${bopTheme}"/>/themes/images/icons/icon1003.png" />
									导出
							</a> <a href="#" onclick="rolloutAudit(1)" name="cmdP" id="cmdP">
									<img
									src="<c:out value="${bopTheme}"/>/themes/images/icons/icon1017.png" />
									审核通过
							</a> <a href="#" onclick="rolloutAudit(0)" name="cmdR" id="cmdR">
									<img
						...
						<table class="lessGrid" cellspacing="0" rules="all" border="0"
							...
							<tr class="lessGrid head">
								<th width="3%" style="text-align: center"><input
									...
									type="checkbox" onClick="checkAll(this,'selects')" /></th>
								...
								<th style="text-align: center">分子</th>
								<th style="text-align: center">分母</th>
								<th style="text-align: center">调整值</th>
								<th style="text-align: center">操作日期</th>
								<th style="text-align: center">机构汇总编号</th>
							</tr>
							<%  List<Proportionality> list=(List)request.getAttribute("RolloutAudit");
							...
							<tr>
								<td><input type="checkbox"
									...
									value="<%= p.getRelationInstId()+"-"+p.getYearMonth()+"-"+p.getResult()+"-"+p.getOperateDate()%>" />
								</td>
								...
								<td style="text-align: center"><%=  p.getDividend()%></td>
								<td style="text-align: center"><%=  p.getDivisor()%></td>
