...
<script type="text/javascript">
	...
	function createTaxDiskInfo(){
		var billIds = document.getElementsByName("selectBillIds");//票据ID
		var fapiaoTypes = document.getElementsByName("fapiaoTypes");//发票类型 0-专票 1-普票
			
		alert(billIds[0].value+"---");
		alert(fapiaoTypes[0].value+"+++");
			
		var fapiaoType = "";
		if(fapiaoTypes.length>0){
			fapiaoType = fapiaoTypes[0].value;//1
		}
		var ids="";
		...
		alert("进...");
		/* 测试 与核心交互 程 */
		//billInterface.createBill2({ids:ids,fapiaoType:fapiaoType});//通用调百旺
...
<body onkeydown="enterkey(event)">
					...
					<div class="widthauto1">
						<table id="tbl_query" cellpadding="0" cellspacing="0" width="100%"
							...
							<tr>
								...
								<td><select name="billInfo.dsouRce" style="width: 135px">
	...
	%>
								...
								<td align="center"><a
									href="viewTrans.action?curPage=<s:property value='paginationList.currentPage'/>&billId=<%=BeanUtils.getValue(bill,"billId")%>&dsource=<%=bill.getDsouRce()%>">
										<img
