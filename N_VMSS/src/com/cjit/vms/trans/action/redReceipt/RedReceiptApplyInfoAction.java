...
import com.cjit.ws.service.request.ReqCore;
...
public class RedReceiptApplyInfoAction extends DataDealAction {
	...
	public String billRedIssueList() {
		...
		redReceiptApplyInfo.setBillBeginDate(request.getParameter("billBeginDate"));
	...
	public String testRed() {
		...
		System.err.println("=====!!!!");
		RedReceiptApplyInfo redReceiptApplyInfo = new RedReceiptApplyInfo();
		redReceiptApplyInfo.setBillCode(request.getParameter("billCode"));
		redReceiptApplyInfo.setBillNo(request.getParameter("billNo"));
		redReceiptApplyInfo.setNoticNo(request.getParameter("notic"));
		chanNelList = this.vmsCommonService.saveRedNotic(redReceiptApplyInfo);
		return SUCCESS;
	}
	...
	public void redReceiptBillPriintToExcel() throws IOException, RowsExceededException, WriteException {
		redReceiptApplyInfo.setDatastatus(DataUtil.BILL_STATUS_5);
		List resultsList = redReceiptApplyInfoService.findRedReceiptList(redReceiptApplyInfo);
	...
	public String viewPrintTrans() {
		try {
			...
			redReceiptApplyInfoService.findTransByBillId(billId, paginationList);
	...
	public String listRedReceipt() {
		...
		try {
			if (null == request.getParameter("msg") || !request.getParameter("msg").equals("message")) {
				this.request.getSession().removeAttribute("message");
				this.message = "";
			}
			User currentUser = this.getCurrentUser();
			if (null != redReceiptApplyInfo && redReceiptApplyInfo.getBillId() != null) {
				redReceiptApplyInfo = null;
			}
			List lstAuthInstId = new ArrayList();
			this.getAuthInstList(lstAuthInstId);
			redReceiptApplyInfo.setLstAuthInstId(lstAuthInstId);
			redReceiptApplyInfo.setdSource(request.getParameter("dSource"));
			billInfoList = redReceiptApplyInfoService.findRedReceiptList("redReceiptList", redReceiptApplyInfo,
			...
			logManagerService.writeLog(request, this.getCurrentUser(), "0001.0020", "红冲", "销项税管理", "查询可供进行红冲申请的票据信息列表",
					"1");
			return SUCCESS;
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void redReceiptBillToExcel() throws Exception {
		...
		if (null != type && type.equals("redReceiptExcel")) {
			...
			billInfoList = redReceiptApplyInfoService.findRedReceiptList("findRedReceipt", redReceiptApplyInfo,
					currentUser.getId(), null);
		} else {
			billInfoList = redReceiptApplyInfoService.findRedReceiptList("findRedReceiptApprove", redReceiptApplyInfo,
					currentUser.getId(), null);
		}
		...
		try {
			...
			for (int i = 0; i < billInfoList.size(); i++) {
				bill = new BillInfo();
				bill = billInfoService.findBillInfo1(((RedReceiptApplyInfo) billInfoList.get(i)).getBillId());
				ck: if (bill != null) {
					for (int k = 0; k < list.size(); k++) {
						if (((BillInfo) list.get(k)).getBillId().equals(bill.getBillId())) {
							break ck;
						}
					}
					list.add(bill);
	...
	private void setWritableSheetForRedReceiptApprove(WritableSheet ws, Map o, int column) throws WriteException {
		...
		// applyBillDate
		Label cell2 = new Label(1, column, o.get("applyBillDate") == null || o.get("applyBillDate").equals("") ? ""
				: o.get("applyBillDate").toString(), tempCellFormat);
		// billDate
		...
		// Label cell18 = new Label(17, column, o.get("insureId").toString(),
		// tempCellFormat);
		ws.addCell(cell1);
		ws.addCell(cell2);
		ws.addCell(cell3);
	...
	// 红冲导出
	private void setWritableSheetForRedReceipt(WritableSheet ws, Map o, int column) throws WriteException {
		...
		Label cell6 = new Label(5, column,
				o.get("noticeNe") == null || o.get("noticeNe").equals("") ? "" : o.get("noticeNe").toString(),
				tempCellFormat);
		...
		Label cell12 = new Label(11, column,
				o.get("oriBillCode") == null || o.get("oriBillCode").equals("") ? "" : o.get("oriBillCode").toString(),
				tempCellFormat);
		Label cell13 = new Label(12, column,
				o.get("oriBillNo") == null || o.get("oriBillNo").equals("") ? "" : o.get("oriBillNo").toString(),
				tempCellFormat);
		Label cell14 = new Label(13, column, o.get("oriBillAmtSum") == null || o.get("oriBillAmtSum").equals("") ? ""
				: o.get("oriBillAmtSum").toString(), tempCellFormat);
		Label cell15 = new Label(14, column, o.get("oritaxAmtSum") == null || o.get("oritaxAmtSum").equals("") ? ""
				: o.get("oritaxAmtSum").toString(), tempCellFormat);
		Label cell16 = new Label(15, column,
				o.get("oriTaxRate") == null || o.get("oriTaxRate").equals("") ? "" : o.get("oriTaxRate").toString(),
				tempCellFormat);
		Label cell17 = new Label(16, column,
				o.get("oriSumAmt") == null || o.get("oriSumAmt").equals("") ? "" : o.get("oriSumAmt").toString(),
				tempCellFormat);
		...
		Label cell18 = new Label(17, column,
				...
				tempCellFormat);
		// taxAmtSum
		Label cell19 = new Label(18, column,
				o.get("taxAmtSum") == null || o.get("taxAmtSum").equals("") ? "" : o.get("taxAmtSum").toString(),
				tempCellFormat);
		// taxAmtSum
		/*
	...
	public String creatWorkCheck() {
		String billId = request.getParameter("billId");
		billInfo = billInfoService.findBillById(billId);
		String result = billInfo.getFapiaoType();
		// response.getWriter().print("返回值");
		try {
	...
	public void exportToWord() throws UnsupportedEncodingException, IOException {
		...
		List list = billInfoService.findRedReceiptList("findRedReceiptApprove1", rrai, currentUser.getId(),
				paginationList);
		redReceiptApplyInfo = (RedReceiptApplyInfo) list.get(0);
		map.put("searchCondition", "t.BILL_NO ='" + redReceiptApplyInfo.getOriBillNo() + "' and t.BILL_CODE = '"
				+ redReceiptApplyInfo.getOriBillCode() + "'");

		Map idmap = new HashMap();
		idmap.put("BILL_NO", redReceiptApplyInfo.getOriBillNo());
		idmap.put("BILL_CODE", redReceiptApplyInfo.getOriBillCode());

		List ticketList = billInfoService.findSpecialTicketById(idmap);
		...
		map.put("dealNo", "");
		map.put("buySellInd", specialTicket.getBuySellInd()); // 购方/销方 0/1
		map.put("level1Option", specialTicket.getLevel1Option()); // 一级选项 对于购方
																	// 选择已抵扣后需要对二级选项进行选择
																	// 购买方：1.已抵扣
																	// 2.未抵扣
																	// 销售方：1.买方发票拒收发票
																	// 2.发票尚未收付
		map.put("level2Option", specialTicket.getLevel2Option()); // 二级选项
																	// 对于购方选择未抵扣
																	// 需要填写该栏位
																	// 1.无法认证
																	// 2.纳税人识别号认证不符
	...
	public String cancelRedReceipt() {
		...
		if (bill != null) {
			...
			if (StringUtil.isNotEmpty(billCode) && StringUtil.isNotEmpty(billNo)) {
				...
				map1.put("dataStatus", DataUtil.BILL_STATUS_16);
				map1.put("billId", billId);
				list.add(map1);
				Map<String, String> map2 = new HashMap<String, String>();
				map2.put("dataStatus", DataUtil.BILL_STATUS_20);
				map2.put("oriBillCode", billCode);
				map2.put("oriBillNo", billNo);
				list.add(map2);
	...
	private void setWritableSheet(WritableSheet ws, RedReceiptApplyInfo redReceiptApplyInfo, int column)
			throws WriteException {
		int i = 0;
		Label cell1 = new Label(i++, column, redReceiptApplyInfo.getApplyDate(), JXLTool.getContentFormat());
		Label cell2 = new Label(i++, column, redReceiptApplyInfo.getBillDate(), JXLTool.getContentFormat());
		Label cell3 = new Label(i++, column, redReceiptApplyInfo.getCustomerName(), JXLTool.getContentFormat());
		Label cell4 = new Label(i++, column, redReceiptApplyInfo.getCustomerTaxno(), JXLTool.getContentFormat());
		Label cell5 = new Label(i++, column, Integer.toString(column), JXLTool.getContentFormat());
		...
		Label cell13 = new Label(i++, column, redReceiptApplyInfo.getTaxAmtSum().toString(),
				JXLTool.getContentFormat());
		Label cell14 = new Label(i++, column, redReceiptApplyInfo.getSumAmt().toString(), JXLTool.getContentFormat());
		Label cell15 = new Label(i++, column, DataUtil.getIsHandiworkCH(redReceiptApplyInfo.getIsHandiwork()),
				JXLTool.getContentFormat());
		Label cell16 = new Label(i++, column, DataUtil.getIssueTypeCH(redReceiptApplyInfo.getIssueType()),
				JXLTool.getContentFormat());
		Label cell17 = new Label(i++, column, DataUtil.getFapiaoTypeCH(redReceiptApplyInfo.getFapiaoType()),
				JXLTool.getContentFormat());
		Label cell18 = new Label(i++, column, DataUtil.getDataStatusCH(redReceiptApplyInfo.getDatastatus(), "BILL"),
				JXLTool.getContentFormat());
	 ...
	 *             导出红票开具
	 */
	public void redReceiptBillIssueToExcel() throws IOException, RowsExceededException, WriteException {
		// redReceiptApplyInfo.setDatastatus(DataUtil.BILL_STATUS_3);
		redReceiptApplyInfo.setDatastatus(DataUtil.BILL_STATUS_21); // 红冲审核通过
		List resultsList = redReceiptApplyInfoService.findRedReceiptList(redReceiptApplyInfo);
	...
	private void setWritIssueableSheet(WritableSheet ws, RedReceiptApplyInfo redReceiptApplyInfo, int column)
		...
		Label cell16 = new Label(i++, column, DataUtil.getFapiaoTypeCH(redReceiptApplyInfo.getFapiaoType()),
				JXLTool.getContentFormat());
		Label cell17 = new Label(i++, column, DataUtil.getDataStatusCH(redReceiptApplyInfo.getDatastatus(), "BILL"),
				JXLTool.getContentFormat());
	...
	public void getRegisteredInfo() throws Exception {
		...
		// 查询空白发票作废数量
		Long invalidInvoiceNum = redReceiptApplyInfoService.findInvalidInvoiceCount("200", fapiaoType);
		this.request.setAttribute("invalidInvoiceNum", invalidInvoiceNum);

		// 获取注册码
		String registeredInfo = redReceiptApplyInfoService.findRegisteredInfo(taxDiskNo);
		...
		// 查询税控盘口令和证书口令
		TaxDiskInfo taxDiskInfo = redReceiptApplyInfoService.findTaxDiskInfoByTaxDiskNo(taxDiskNo);
		if (taxDiskInfo == null) {
			result = "taxPwdError";
			printWriterResult(result);
			return;
	...
	public void issueRedBill() throws Exception {
		...
		// 获取作废的空白发票 ----需发票类型
		List invalidInvoiceList = redReceiptApplyInfoService.findInvalidPaperInvoice("200", "发票类型");
		...
		// 红冲开具发票商品信息
		List billItemList = redReceiptApplyInfoService.findBillItemByBillId(billId);
	...
	public void updateIssueRedBillResult() throws Exception {
		...
		map.put("billDate", new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()));
		map.put("dataStatus", DataUtil.BILL_STATUS_99);
		billInfoService.updateRedBillInfo(map);
		List<BillEntity> billEntities = billInfoService.findBillRedInfo(billId);
		
		for (BillEntity billEntity : billEntities) {
			billInfoService.updateBillStatisticsCount(billEntity.getBillCode(),billEntity.getBillNo());
		...
		try {
			...
			if ("1".equals(callBack.getResulttype())) {
				...
				coreMessage = new AjaxReturn(false, "开票成功，回写核心失败");
			}
			coreMessage = new AjaxReturn(true, "开票成功，回写核心成功");
		} catch (Exception e) {
			coreMessage = new AjaxReturn(false, "开票成功，回写核心失败");
		} finally {
			...
			if ("".equals(errorMessage)) {// 校验完成没有错误则生成报文
				...
				if (transList.size() == 1) {
					for (Iterator it = transList.iterator(); it.hasNext();) {
						...
						String transId = trans.getTransId().substring(0,
								trans.getTransId().lastIndexOf(trans.getTransType()));
				...
				if (XmlUtil.validateXML(requestXml)) {
					logManagerService.writeLog(this.request, this.getCurrentUser(), serialno, "红票开具申请接口", "请求报文",
							requestXml, "1");
					...
					if ("LSP".equals(billInfo.getDsouRce())) { // 个险
						...
						/*
						 * VatServicePortType_VatServicePort_Client client = new
						 * VatServicePortType_VatServicePort_Client();
						 * responseXml = client.invoke(interfaceParams[0],
						 * requestXml, interfaceParams[1], interfaceParams[2]);
						 */
						// 个险专属的方法，里面有些如命名空间或者调用方法个险若有修改，则需检查生成的客户端所有java类都需要修改
						VatServiceClient vatServiceClient = new VatServiceClient();
						responseXml = vatServiceClient.invoke(interfaceParams[0], requestXml, interfaceParams[1],
								interfaceParams[2]);
					} else if ("LIS".equals(billInfo.getDsouRce())) { // 团险
						Axis2Client axis2Client = new Axis2Client();
						responseXml = axis2Client.invokeRPCClient(interfaceParams[0], requestXml, interfaceParams[1],
								interfaceParams[2]);
					}
					// //本地测试用
					...
					logManagerService.writeLog(this.request, this.getCurrentUser(), serialno, "红票开具申请接口", "响应报文",
							responseXml, "1");
	...
	public String showOCXRedstring() {
		try {
			...
			sbInner.append(billInfo.getBillId()).append("^").append(billInfo.getBillCode()).append("^")
	...
	public String listRedReceiptApply() {
		...
		try {
			...
			if (null != request.getParameter("back") && request.getParameter("back").equals("back")) {
			...
			/* redReceiptApplyInfo = new RedReceiptApplyInfo(); */
			redReceiptApplyInfo.setLstAuthInstId(lstAuthInstId);
			redReceiptApplyInfo.setdSource(request.getParameter("dSource")); //第一次进为null
			redReceiptApplyInfo.setBillBeginDate(request.getParameter("billBeginDate")); // WJM  第一次进为null
			redReceiptApplyInfo.setBillEndDate(request.getParameter("billEndDate")); //第一次进为null
			redReceiptApplyInfo.setHissDteBegin(request.getParameter("hissDteBegin")); //第一次进为null
			redReceiptApplyInfo.setHissDteEnd(request.getParameter("hissDteEnd")); //第一次进为null
			redReceiptApplyInfo.setFapiaoType(request.getParameter("fapiaoType")); //第一次进为null
			
			chanNelList = this.vmsCommonService.findCodeDictionary("CHANNEL_TYPE"); //null
			billFreqList = this.vmsCommonService.findCodeDictionary("PAYMENT_FREQUENCY"); //null
			chargesList = this.vmsCommonService.findCodeDictionary("CHARGES_TYPE"); //null
			dsouRceList = this.vmsCommonService.findCodeDictionary("DATA_SOURCE"); //null
			//2018-06-08修改
			this.paginationList.setShowCount("true");
			billInfoList = redReceiptApplyInfoService.findRedReceiptApplyList("findRedReceipt", redReceiptApplyInfo,
					currentUser.getId(), paginationList);
			this.request.setAttribute("paginationList", paginationList);
			this.request.setAttribute("configCustomerFlag", this.configCustomerFlag);
			logManagerService.writeLog(request, this.getCurrentUser(), "0001.0020", "红冲申请", "销项税管理",
					"查询可供进行红冲申请的票据信息列表", "1");
			return SUCCESS;
		} catch (Exception e) {
			e.printStackTrace();
		...
		if (result.equals("17")) {
			...
			for (int i = 0; i < ids.length; i++) {
				...
				if (!ids[i].equals("") && ids[i] != null) {
					...
					RedReceiptApplyInfo rrai = redReceiptApplyInfoService.findListByBillId(ids[i], map);
					...
					List itemList = redReceiptApplyInfoService.findBillItemInfoList(tempItem);
					for (int j = 0; j < itemList.size(); j++) {
						bii = (BillItemInfo) itemList.get(j);
						redReceiptApplyInfoService.deleteBillItemInfo(rrti.getBillId(), bii.getBillItemId());
					}
					for (int k = 0; k < list.size(); k++) {
	...
	public String redReceiptApply() {
		...
		List list = redReceiptApplyInfoService.findRedReceiptTransByIds(billId, null, selectBillIds);
		...
		// 获取所有红冲后的新bill
		List redBill = redReceiptApplyInfoService.findRedBillByOriBillId(billId);
		...
		bill.setBalance(bill.getBalance() == null ? bill.getSumAmt().add(redSumAmt) : bill.getBalance().add(redSumAmt));
	...
	public String transBillMergeOrSplit(List selTransList) {
		...
		// 获取首个transBill
		RedReceiptTransInfo firstTransBill = (RedReceiptTransInfo) selTransList.get(0);
		...
		for (int i = 0; i < selTransList.size(); i++) {
			...
			boolean isSameItemId = billItemId.equalsIgnoreCase(indexTransBill.getBillItemId());
			...
			if (isFirst) {
				if (isLast) {
					// 保存新数据
					this.createRedBill(strOldBillId, strBillId, strBillItemId, transBillList);
					continue;
				} else {
					continue;
				}
	 ...
	 */
	private void createRedBill(String strOldBillId, String newBillId, String newBillItemId, List transBillList) {
		// transBill逐个取出，各金额置负值，保存;相应billItem中各金额算出
		RedReceiptTransInfo transBill = (RedReceiptTransInfo) transBillList.get(0);
		// 获取原billId
		...
		for (int i = 0; i < transBillList.size(); i++) {
			...
			// 插入交易票据对应信息
			transInfoService.saveTransBill(transBill.getTransId(), newBillId, newBillItemId,
		...
		billItem = (BillItemInfo) (redReceiptApplyInfoService.findBillItemInfoList(billItem)).get(0);
		// 更换金额字段值
		...
		if (!strOldBillId.equalsIgnoreCase(newBillId)) {
			...
			List billItemList = redReceiptApplyInfoService.findBillItemInfoList(newBillItem);
			// 合计金额
	...
	public String listRedReceiptApplyupload(){
		if (!sessionInit(true)) {
		...
		String fileName =request.getParameter("fileName")+"/"+request.getParameter("fileName")+"-"+"16";
