...
public class BillInfoAction extends BillAction {
	...
	public String listBillTrack() {
		...
		try {
			...
			billInfoList = billInfoService.findBillInfoList(billInfo, currentUser.getId(), paginationList);
			this.request.setAttribute("billInfoList", billInfoList);
			this.request.getSession().setAttribute("billBeginDate", billInfo.getBillBeginDate());
			this.request.getSession().setAttribute("billEndDate", billInfo.getBillEndDate());
			String currMonth = DateUtils.toString(new Date(), DateUtils.DATE_FORMAT_YYYYMM);
			this.request.setAttribute("currMonth", currMonth);
			logManagerService.writeLog(request, this.getCurrentUser(), "0001.0020", "废票红冲", "销项税管理",
					"查询可供进行废票/红冲处理的票据信息列表", "1");
			return SUCCESS;
		} catch (Exception e) {
			e.printStackTrace();
			logManagerService.writeLog(request, this.getCurrentUser(), "0001.0020", "废票红冲", "销项税管理",
					"查询可供进行废票/红冲处理的票据信息列表", "0");
		...
		try {
			if (this.selectBillIds != null && this.selectBillIds.length > 0) {
				...
				for (int i = 0; i < this.selectBillIds.length; i++) {
					BillInfo bill = billInfoService.findBillInfo(selectBillIds[i]);
					if (bill != null) {
						if (!DataUtil.BILL_STATUS_5.equals(bill.getDataStatus())
								&& !DataUtil.BILL_STATUS_6.equals(bill.getDataStatus())
								&& !DataUtil.BILL_STATUS_7.equals(bill.getDataStatus())
								&& !DataUtil.BILL_STATUS_8.equals(bill.getDataStatus())
								&& !DataUtil.BILL_STATUS_9.equals(bill.getDataStatus())) {
							// 非完成状态票据不可做废票和红冲
							continue;
						}
						...
						logManagerService.writeLog(request, this.getCurrentUser(), "0001.0010", "销项税管理", "票据跟踪",
								"申请对票据ID为(" + billId + ")的票据进行废票处理", "1");
					}
				}
			}
			return SUCCESS;
	...
	public String listBillItemYS() {
		try {
			...
			if (StringUtil.isNotEmpty(billId)) {
				List billItemList = billInfoService.findBillItemInfoList(new BillItemInfo(billId, null));
	...
	public String createBill() {
		...
		try {
			...
			if (org != null) {
				...
				billInfo.setAddressandphone(org.getTaxAddress() + " " + org.getTaxTel());
				billInfo.setBankandaccount(org.getTaxBank() + " " + org.getAccount());
			}
		...
		try {
			...
			if (!"dis".equalsIgnoreCase(this.flag)) {
				...
				if (StringUtil.isEmpty(billItemId)) {
					...
					if ("1".equals(billItem.getRowNature()) && StringUtil.isNotEmpty(billItem.getDisItemId())) {
						// 当前记录为折扣行记录，查询被折扣票据明细信息
						BillItemInfo oriBillItem = this.billInfoService.findBillItemInfo(billItem.getDisItemId());
						if (oriBillItem != null) {
						}
	...
	public String editPreBillItem() {
		try {
			...
			if (StringUtil.isNotEmpty(billId) && StringUtil.isNotEmpty(billItemId)) {
				List billItemList = billInfoService.findBillItemInfoList(new BillItemInfo(billId, billItemId));
	...
	public String savePreBillItem() {
		try {
			...
			if ("Y".equals(taxFlax)) {
				BigDecimal amtBig = billItem.getAmt().subtract(billItem.getTaxAmt());
	...
	public String disBillItem() {
		try {
			if (StringUtil.IsEmptyStr(billItem.getBillItemId()) && StringUtil.isNotEmpty(billItem.getDisItemId())
					&& billItem.getDiscountRate() != null) {
				// 查询原票据明细
				BillItemInfo oriBillItem = billInfoService.findBillItemInfo(billItem.getDisItemId());
				...
				disBillItem.setAmt(new BigDecimal(0.0).subtract(
						oriBillItem.getAmt().multiply(new BigDecimal(1).subtract(billItem.getDiscountRate()))));
				disBillItem.setTaxRate(oriBillItem.getTaxRate());
				disBillItem.setTaxAmt(new BigDecimal(0.0).subtract(
						oriBillItem.getTaxAmt().multiply(new BigDecimal(1).subtract(billItem.getDiscountRate()))));
	...
	public String deleteBillItem() {
		try {
			...
			// 查询准备删除的票据明细
			BillItemInfo billItem = billInfoService.findBillItemInfo(billItemId);
			// 依该票据的商品行性质进行不同操作
			if ("0".equals(billItem.getRowNature())) {
				...
				// 查询对应折扣行记录
				List billItemList = billInfoService.findBillItemInfoList(billItemTemp);
				if (CollectionUtil.isNotEmpty(billItemList)) {
					for (Iterator t = billItemList.iterator(); t.hasNext();) {
						...
						if (disBillItem != null && StringUtil.isNotEmpty(disBillItem.getBillItemId())) {
							// 删除折扣行记录
							billInfoService.deleteBillItemInfo(null, disBillItem.getBillItemId());
						}
					}
				}
				billInfoService.deleteBillItemInfo(null, billItemId);
	...
	public void loadOurInfo() {
		try {
			...
			if (org != null) {
				...
				sb.append(org.getTaxperName()).append("###").append(org.getTaxperNumber()).append("###")
						.append(org.getTaxAddress()).append("###").append(org.getTaxTel()).append("###")
						.append(org.getTaxBank()).append("###").append(org.getAccount());
			}
	...
	public void loadTransTypeInfo() {
		try {
			...
			if (StringUtil.isNotEmpty(transType)) {
				Business buss = businessService.findBusiness(new Business(null, transType));
				if (buss != null) {
					// 是否含税
					...
					if (buss.getTaxRate() != null) {
						...
						if ("1".equals(buss.getHasTax()) && StringUtil.isNotEmpty(price)
							...
							// 计算税额
							BigDecimal bdTaxAmt = bdAmount.multiply(buss.getTaxRate());
							// 金额;税额
							sb.append(bdAmount.toString()).append(";").append(bdTaxAmt.toString());
						} else {
							// 金额;税额
	...
	public void loadBillAmtInfo() {
		try {
			...
			if (StringUtil.isNotEmpty(billId)) {
				...
				sb.append(bdAmtSum.toString()).append("###").append(bdTaxAmtSum.toString()).append("###")
						.append(bdSumAmt.toString());
			}
			this.response.setContentType("text/html; charset=UTF-8");
				...
				for (Iterator t = itemList.iterator(); t.hasNext();) {
					...
					if ("0".equals(item.getRowNature())) {
						...
						for (Iterator t1 = itemList.iterator(); t1.hasNext();) {
							...
							// 计算打折后金额和税金
							if ("1".equals(item1.getRowNature()) && item.getBillItemId().equals(item1.getDisItemId())) {
								item.setDiscountRate(item1.getDiscountRate());
								item.setAmt(item.getAmt().subtract(item1.getAmt()));
								item.setTaxAmt(item.getTaxAmt().subtract(item1.getTaxAmt()));
								billItemList.add(item);
								break;
							} else {
								continue;
	...
	public String writeOffBill() {
		try {
			...
			// 票据明细红冲数量
			String[] cancelGoodsNos = request.getParameterValues("cancelGoodsNo");
			// 票据折扣率
			...
			if (billItemIds != null && billItemIds.length > 0) {
				...
				for (int i = 0; i < billItemIds.length; i++) {
					BillItemInfo billItem = this.billInfoService.findBillItemInfo(billItemIds[i]);
					billItemList.add(billItem);
				}
			}
			// 调用方法 生成红冲票据
			boolean isSuccess = this.writeOffBill(billId, noticeNo, remark, billItemList, cancelGoodsNos, discountRates,
		...
		try {
			...
			if ("menu".equalsIgnoreCase(fromFlag)) {
				...
				if (!String.valueOf(this.request.getSession().getAttribute("billBeginDate")).isEmpty()
						&& !"null".equals(String.valueOf(this.request.getSession().getAttribute("billBeginDate")))) {
					this.billInfo
							.setBillBeginDate(String.valueOf(this.request.getSession().getAttribute("billBeginDate")));
				}
				// 申请开票日期(结束)
				if (!String.valueOf(this.request.getSession().getAttribute("billEndDate")).isEmpty()
						&& !"null".equals(String.valueOf(this.request.getSession().getAttribute("billEndDate")))) {
					this.billInfo.setBillEndDate(String.valueOf(this.request.getSession().getAttribute("billEndDate")));
				}
				// 客户名称
				if (!"".equals(this.request.getSession().getAttribute("customerName"))) {
					this.billInfo
							.setCustomerName(String.valueOf(this.request.getSession().getAttribute("customerName")));
				}
				// 客户纳税人识别号
				if (!"".equals(this.request.getSession().getAttribute("customerTaxno"))) {
					this.billInfo
							.setCustomerTaxno(String.valueOf(this.request.getSession().getAttribute("customerTaxno")));
				}
				// 是否手工录入
				...
				billInfo = new BillInfo();
				this.billInfo.setBillBeginDate(this.request.getParameter("billInfo.billBeginDate"));
				this.billInfo.setBillEndDate(this.request.getParameter("billInfo.billEndDate"));
				...
				this.billInfo.setIsHandiwork(this.request.getParameter("billInfo.isHandiwork"));
				...
				this.billInfo.setDsouRce(this.request.getParameter("dsouRce"));
			}

			try {
				printLimitValue = Integer.valueOf(paramConfigVmssService.findvaluebyName("单次打印限值（张）")).intValue();
			...
			// billInfo.setBillEndDate(this.getBillEndDate());
			// 默认按照专用发票查询
			if (billInfo.getFapiaoType() == null || billInfo.getFapiaoType().equals("1")) {
			...
			billInfo.setBillCode(request.getParameter("billInfo.billCode"));
			billInfo.setBillNo(request.getParameter("billInfo.billNo"));
			//计数新增
			paginationList.setShowCount("true");
			billInfoList = billInfoService.findBillInfoListNew(billInfo, currentUser.getId(), paginationList);

			this.request.setAttribute("billInfoList", billInfoList);
			this.request.setAttribute("paginationList", paginationList);

			if (this.billInfo.getBillBeginDate() != null) {
				this.request.getSession().setAttribute("billBeginDate", this.billInfo.getBillBeginDate());
			}
			if (this.billInfo.getBillEndDate() != null) {
				this.request.getSession().setAttribute("billEndDate", this.billInfo.getBillEndDate());
			}
			...
			if (this.billInfo.getDataStatus() != null) {
				this.request.getSession().setAttribute("dataStatus", this.billInfo.getDataStatus());
			}
				...
				for (int i = 0; i < this.selectBillIds.length; i++) {
					...
					if (bill != null) {
						...
						List billItemList = billInfoService
								.findBillItemInfoList(new BillItemInfo(bill.getBillId(), null));
						...
						String invoiceData = FileUtil.createXmlForBillPrint(bill); // 发票数据（XML格式）
						String callResult = callServiceAxis(clientNo, taxMachineIP, sysInvNo, invoiceList, invoiceSplit,
								invoiceConsolidate, invoiceData);
						if (StringUtil.isNotEmpty(callResult)) {
							...
							if ("0".equals(map.get("OperateFlag"))) {
								...
								// 成功
								// 将交易状态改为99
								List transIds = billInfoService.findtransId(billId);
								for (int j = 0; j < transIds.size(); j++) {
									...
									if (transInfo.getBalance().equals(new BigDecimal(0))) {

										transInfoService.updateTransDatastatus(info.getTransId(),
								...
								String invoiceCode = (String) map.get("InvCode");
	...
	public void xxsToExcel() throws Exception {
		try {
			String[] selectBillIds = (String[]) request.getAttribute("selectBillIds");
			...
			billInfo.setBillBeginDate(this.getBillBeginDate());
			billInfo.setBillEndDate(this.getBillEndDate());
			billInfoList = billInfoService.findBillInfoList(billInfo, currentUser.getId(), selectBillIds);
	...
	private void setWritableSheetNew(WritableSheet ws, BillInfo billInfo, int column) throws WriteException {
		Label cell1 = new Label(0, column, column + "", JXLTool.getContentFormat());
		Label cell2 = new Label(1, column, (String) billInfo.getApplyDate(), JXLTool.getContentFormat());
		Label cell3 = new Label(2, column, (String) billInfo.getBillDate(), JXLTool.getContentFormat());
		Label cell4 = new Label(3, column, (String) billInfo.getCustomerName(), JXLTool.getContentFormat());
		Label cell5 = new Label(4, column, (String) billInfo.getCustomerTaxno(), JXLTool.getContentFormat());
		Label cell6 = new Label(5, column, (String) billInfo.getBillCode(), JXLTool.getContentFormat());
		Label cell7 = new Label(6, column, (String) billInfo.getBillNo(), JXLTool.getContentFormat());
		Label cell8 = new Label(7, column, (String) billInfo.getDrawer(), JXLTool.getContentFormat());
		Label cell9 = new Label(8, column, (String) billInfo.getTaxDiskNo(), JXLTool.getContentFormat());
		Label cell10 = new Label(9, column, (String) billInfo.getMachineNo(), JXLTool.getContentFormat());
		Label cell11 = new Label(10, column, billInfo.getAmtSum() == null ? "" : billInfo.getAmtSum().toString(),
				JXLTool.getContentFormat());
		Label cell12 = new Label(11, column, billInfo.getTaxAmtSum() == null ? "" : billInfo.getTaxAmtSum().toString(),
				JXLTool.getContentFormat());
		Label cell13 = new Label(12, column, billInfo.getSumAmt() == null ? "" : billInfo.getSumAmt().toString(),
				JXLTool.getContentFormat());
		// 是否手工录入 1-自动开票;2-人工审核;3-人工开票
		Label cell14 = new Label(13, column, billInfo.getIsHandiwork().equals("1") ? "自动开票"
				: billInfo.getIsHandiwork().equals("2") ? "人工审核" : "人工开票", JXLTool.getContentFormat());
		// 开具类型1-单笔;2-合并;3-拆分
		Label cell15 = new Label(14, column,
				billInfo.getIssueType().equals("1") ? "单笔" : billInfo.getIssueType().equals("2") ? "合并" : "拆分",
				JXLTool.getContentFormat());
		// 发票类型 0-增值税专用发票;1-增值税普通发票
		Label cell16 = new Label(15, column, billInfo.getFapiaoType().equals("0") ? "增值税专用发票" : "增值税普通发票",
				JXLTool.getContentFormat());
		Label cell17 = new Label(16, column, getDataStatusC(billInfo.getDataStatus()), JXLTool.getContentFormat());
		ws.addCell(cell1);
		ws.addCell(cell2);
		ws.addCell(cell3);
		ws.addCell(cell4);
	...
	public String billsToXml() {
		...
		try {
			...
			String tableId = "";
			String subTableId = "";
			if (billType.equals("1") || billType.equals("4") || billType.equals("5") || billType.equals("6")) {
			...
			if (billType.equals("1")) {
				...
				if (dataList1 != null && dataList1.size() > 0) {
					...
					for (int i = 0; i < dataList1.size(); i++) {
						...
						List subDataList = initXmlSubDatas(bi.getBillId(), subTableId);
						if (subDataList != null && subDataList.size() > 0) {
							for (int j = 0; j < subDataList.size(); j++) {
								BillItemInfo bii = (BillItemInfo) subDataList.get(j);
								// bill_item_info中的子节点
								...
								djEle.setText(bii.getGoodsPrice() == null ? "" : bii.getGoodsPrice().toString());
								// 数量
								Element slEle = sphEle.addElement("Sl");
								slEle.setText(bii.getGoodsNo() == null ? "" : bii.getGoodsNo().toString());
								// 金额
								Element jeEle = sphEle.addElement("Je");
								jeEle.setText(bii.getAmt() == null ? "" : bii.getAmt().toString());
								// 税率
								Element slvEle = sphEle.addElement("Slv");
								slvEle.setText(bii.getTaxRate() == null ? "" : bii.getTaxRate().toString());
								// slvEle.setText("0.17");
							}
						}
					}
					...
					response.setHeader("Content-Disposition", "attachment;filename=" + curDate + instCode + "spec.xml");
					int sum = 0;
					int k = 0;
					...
					JOptionPane.showMessageDialog(null, "导出发票信息未找到", "提示", JOptionPane.ERROR_MESSAGE);
					response.getOutputStream().close();
					return SUCCESS;
				}
			} else {
				...
				if (dataList1 != null && dataList1.size() > 0) {
					...
					response.setContentLength((int) bytes.length);
					// 增值税专用发票 billType=1 增值税普通发票=4 货物运输业增值税专用发票=5 机动车销售统一发票=6
					response.setHeader("Content-Disposition", "attachment;filename=" + curDate + instCode + "comm.xml");
					int sum = 0;
					int k = 0;
					OutputStream output = response.getOutputStream();
					while ((k = inputstream.read(abyte0, 0, bufferSize)) > -1) {
	...
	private List initXmlDatas1(String customer_taxno, String tableId) {
		// 增值税专用发票 billType=1 增值税普通发票=4
		List dataList = billInfoService.findTaxpayerTypeDatas(customer_taxno, tableId);
		return dataList;
	}
	...
	private List initXmlDatas2(String customer_taxno, String tableId) {
		// 增值税专用发票 billType=1 增值税普通发票=4
		List dataList = billInfoService.findTaxpayerTypeDatas2(customer_taxno, tableId);
		return dataList;
	}
	...
	private void writeFile(String filePath, String fileName, Document document) throws Exception {
		// document到文件
		DocXmlUtil.fromDocumentToFile(document, fileName, CharacterEncoding.GBK);
	}

	public static String callServiceAxis(String clientNo, String taxMachineIP, String sysInvNo, String invoiceList,
			String invoiceSplit, String invoiceConsolidate, String invoiceData) throws Exception {
		try {
			...
			call.setOperationName(new QName("http://tempuri.org/", "CallService"));
			// 该方法需要的参数
			call.addParameter(new QName("http://tempuri.org/", "CLIENTNO"), org.apache.axis.encoding.XMLType.XSD_STRING,
					javax.xml.rpc.ParameterMode.IN);
			call.addParameter(new QName("http://tempuri.org/", "TaxMachineIP"),
					org.apache.axis.encoding.XMLType.XSD_STRING, javax.xml.rpc.ParameterMode.IN);
			call.addParameter(new QName("http://tempuri.org/", "SysInvNo"), org.apache.axis.encoding.XMLType.XSD_STRING,
					javax.xml.rpc.ParameterMode.IN);
			call.addParameter(new QName("http://tempuri.org/", "InvoiceList"),
					org.apache.axis.encoding.XMLType.XSD_STRING, javax.xml.rpc.ParameterMode.IN);
			call.addParameter(new QName("http://tempuri.org/", "InvoiceSplit"),
					org.apache.axis.encoding.XMLType.XSD_STRING, javax.xml.rpc.ParameterMode.IN);
			call.addParameter(new QName("http://tempuri.org/", "InvoiceConsolidate"),
					org.apache.axis.encoding.XMLType.XSD_STRING, javax.xml.rpc.ParameterMode.IN);
			...
			Object[] fn01 = { clientNo, taxMachineIP, sysInvNo, invoiceList, invoiceSplit, invoiceConsolidate,
					invoiceData };
	...
	public static Map receiveXml(String feedbackXml) {
		Map map = new HashMap();
		if (feedbackXml.indexOf("<SysInvNo>") > 0 && feedbackXml.indexOf("</SysInvNo>") > 0) {
			String str = feedbackXml.substring(feedbackXml.indexOf("<SysInvNo>") + "<SysInvNo>".length(),
					feedbackXml.indexOf("</SysInvNo>"));
			map.put("SysInvNo", str);
		}
		if (feedbackXml.indexOf("<InvCode>") > 0 && feedbackXml.indexOf("</InvCode>") > 0) {
			String str = feedbackXml.substring(feedbackXml.indexOf("<InvCode>") + "<InvCode>".length(),
					feedbackXml.indexOf("</InvCode>"));
			map.put("InvCode", str);
		}
		if (feedbackXml.indexOf("<InvNo>") > 0 && feedbackXml.indexOf("</InvNo>") > 0) {
			String str = feedbackXml.substring(feedbackXml.indexOf("<InvNo>") + "<InvNo>".length(),
					feedbackXml.indexOf("</InvNo>"));
			map.put("InvNo", str);
		}
		if (feedbackXml.indexOf("<InvDate>") > 0 && feedbackXml.indexOf("</InvDate>") > 0) {
			String str = feedbackXml.substring(feedbackXml.indexOf("<InvDate>") + "<InvDate>".length(),
					feedbackXml.indexOf("</InvDate>"));
			map.put("InvDate", str);
		}
		if (feedbackXml.indexOf("<CancelDate>") > 0 && feedbackXml.indexOf("</CancelDate>") > 0) {
			String str = feedbackXml.substring(feedbackXml.indexOf("<CancelDate>") + "<CancelDate>".length(),
					feedbackXml.indexOf("</CancelDate>"));
			map.put("CancelDate", str);
		}
		if (feedbackXml.indexOf("<OperateFlag>") > 0 && feedbackXml.indexOf("</OperateFlag>") > 0) {
			String str = feedbackXml.substring(feedbackXml.indexOf("<OperateFlag>") + "<OperateFlag>".length(),
					feedbackXml.indexOf("</OperateFlag>"));
			map.put("OperateFlag", str);
		}
		if (feedbackXml.indexOf("<PrintFlag>") > 0 && feedbackXml.indexOf("</PrintFlag>") > 0) {
			String str = feedbackXml.substring(feedbackXml.indexOf("<PrintFlag>") + "<PrintFlag>".length(),
					feedbackXml.indexOf("</PrintFlag>"));
			map.put("PrintFlag", str);
		}
		if (feedbackXml.indexOf("<returnmsg>") > 0 && feedbackXml.indexOf("</returnmsg>") > 0) {
			String str = feedbackXml.substring(feedbackXml.indexOf("<returnmsg>") + "<returnmsg>".length(),
					feedbackXml.indexOf("</returnmsg>"));
	...
	public String redReceiptSpecialSave() {
		Map map = new HashMap();
		map.put(" searchCondition", "t.BILL_NO ='" + specialTicket.getBillNo() + "' and t.BILL_CODE = '"
				+ specialTicket.getBillCode() + "'");
		...
		if (null != commit && commit.equals("commit")) {
			BillInfo bill = billInfoService.findBillInfo1(specialTicket.getBillId());
			if (bill == null) {
	...
	/**
	 * 发票红冲 1.红冲当前票据 2.红冲票据对应的交易 3.修改负数发票信息为红冲待开票 4.回写核心系统
	 * 5.提示用户当前交易下有哪些发票需要红冲或作废
	 * 
	 * @return
	 * @throws Exception
	 */
	public void billRedCancel() throws Exception {
		AjaxReturn coreMessage = null;
		String billId = request.getParameter("billId");
		List<BillEntity> billEntities = billInfoService.findBillRedInfo(billId);
		List<TransInfo> transList=new ArrayList<TransInfo>();
		
		if (billEntities==null&&billEntities.size()==0) {
			return;
		}
		BillEntity bill=billEntities.get(0);
		String amt = bill.getAmt();
		String taxAmt=bill.getTaxAmt();
		//红票税额
		BigDecimal hcSyTaxAmt=new BigDecimal(taxAmt);
		//红票价税合计
		BigDecimal hcSyAmt=new BigDecimal(amt);
		for (BillEntity billEntity : billEntities) {
			billEntity.setDataStatus(WebServiceUtil.BILL_RED);
			//当前交易已开票税额
			BigDecimal ykTaxAmt=billEntity.getTaxCny().subtract(billEntity.getTaxCnyBalance());
			//当前交易已开票加税合计
			BigDecimal ykAmt=billEntity.getAmtCny().subtract(billEntity.getBalance());
			TransInfo transInfo=new TransInfo();
			transInfo.setTransId(billEntity.getTransId());
			transInfo.setDataStatus(DataUtil.TRANS_STATUS_3);
			if (ykAmt.compareTo(hcSyAmt)==-1) {
				hcSyAmt=hcSyAmt.subtract(ykAmt);
				transInfo.setBalance(billEntity.getAmtCny());
				hcSyTaxAmt=hcSyTaxAmt.subtract(ykTaxAmt);
				transInfo.setTaxCnyBalance(billEntity.getTaxCny());
				transList.add(transInfo);
			}else {
				transInfo.setBalance(hcSyAmt);
				transInfo.setTaxCnyBalance(hcSyTaxAmt);
				transList.add(transInfo);
				break;
			}
		}
		//查找该发票对应的负数发票billId
		String fsBillId = billInfoService.findFSBill(billId);
		...
		map2.put("dataStatus", DataUtil.BILL_STATUS_21);
		map2.put("billId", fsBillId);
		list.add(map2);
		// 更改待红冲发票和负数发票状态
		billInfoService.updateBillStatus(list);
		billInfoService.updateBillStatisticsCount(billId);
		
		// 更新红冲票据对应的交易的未开票金额
		billInfoService.updateBillTransInfo(transList);
		...
		try {
			...
			submitData.setRequestJSON(com.cjit.webService.client.Util.JsonUtil.billIssueJson(billEntities, "TRUE", true));
			SubmitDataResponse submitDataResponse = invoicePrintServiceStub.submitData(submitData);
			HeXinCallBack callBack = com.cjit.webService.client.Util.JsonUtil
					.callBacks(submitDataResponse.getSubmitDataReturn());
			if ("1".equals(callBack.getResulttype())) {
				for (BillEntity billEntity : billEntities) {
					billEntity.setErrorInfo(callBack.getErrorInfo());
				}
				coreMessage = new AjaxReturn(false, "红冲成功，回写核心失败");
			}
			coreMessage = new AjaxReturn(true, "红冲成功，回写核心成功");
		} catch (Exception e) {
			coreMessage = new AjaxReturn(false, "红冲成功，回写核心失败");
		} finally {
	...
	public String listBillItem1() {
		try {
			...
			String isHandiwork = request.getParameter("isHandiwork");
			if (StringUtil.isNotEmpty(billId)) {
				List billItemList = billInfoService.findBillItemInfoList1(new BillItemInfo(billId, null));
				this.request.setAttribute("billItemList", billItemList);
				this.request.setAttribute("billId", billId);
			}
			this.request.setAttribute("isHandiwork", isHandiwork);
	...
	public void exportApplyInfo() {
		...
		try {
			...
			configuration.setClassForTemplateLoading(this.getClass(), "/com/cjit/vms/trans/util");
			Template t = null;
			try {
				// test.ftl为要装载的模板
	...
	public String showOCXstring() throws Exception {
		try {
			...
			int printLimitValue = Integer.parseInt(request.getParameter("printLimitValue"));
			...
			if (selectedBillIds != null && selectedBillIds.length > 0) {
				List bills = billInfoService.findBillInfoByIDFaPiaoType(selectedBillIds, faPiaoType);

				if (bills.size() == 1) {
					BillInfo billIn = (BillInfo) bills.get(0);
					List list = paperInvoiceTrackService.findReREByBillCodeAndBillNo(billIn.getBillCode(),
							billIn.getBillNo());
					if (list.size() == 0) {
						...
						sbInner.append(billIn.getBillId()).append("^").append(billIn.getBillCode()).append("^")
								.append(billIn.getBillNo());
					...
					if (list.size() == 0) {
						...
						sbInner.append('0').append("^");
						sbInner.append(billIn.getBillId()).append("^").append(billIn.getBillCode()).append("^")
								.append(billIn.getBillNo()).append("|");
						...
						for (int i = 0; i < bills.size() - 1; i++) {
							...
							BigDecimal temId = new BigDecimal(Integer.valueOf(bill.getBillNo()).intValue());
							BigDecimal billNo = new BigDecimal(Integer.valueOf(bi.getBillNo()).intValue());
							if (list.size() == 0) {
								lingyong.append(billIn.getBillNo());
								falg = true;
								break;
							} else {
								if (i < bills.size() - 1 && temId.subtract(billNo).intValue() == 1
										&& bi.getBillCode().equals(bill.getBillCode())) { // 增加发票代码的条件
									// 注册码|发票类型(0:专,1:普)|发票数量|发票ID^发票代码^发票号码|
									// 发票ID^发票代码^发票号码为循环域
									sbInner.append('0').append("^");
									sbInner.append(bill.getBillId()).append("^").append(bill.getBillCode()).append("^")
			...
			String result = "";
			if (printFalg) {
				result = "1" + "+" + sbInner.toString() + "+" + lianxu.toString() + "+" + String.valueOf(j) + "+"
						+ billPrintId;
			} else {
				result = "0" + "+" + lingyong;
			}
	...
	private void setWritableSheet(WritableSheet ws, Map o, int column) throws WriteException {
		Label cell1 = new Label(0, column, (String) o.get("customerName"), JXLTool.getContentFormat());
		Label cell2 = new Label(1, column, (String) o.get("customerTaxno"), JXLTool.getContentFormat());
		Label cell3 = new Label(2, column, (String) o.get("amtSum").toString(), JXLTool.getContentFormat());
		Label cell4 = new Label(3, column, (String) o.get("taxAmtSum").toString(), JXLTool.getContentFormat());
		Label cell5 = new Label(4, column, (String) o.get("sumAmt").toString(), JXLTool.getContentFormat());
		ws.addCell(cell1);
		ws.addCell(cell2);
		ws.addCell(cell3);
		ws.addCell(cell4);
	...
	public void setPaperInvoiceTrackService(PaperInvoiceTrackService paperInvoiceTrackService) {
