    ...
    return function () {
        return func.apply(context, arguments); 
			...
			$(function(){
				 ...
				 $.get("top.action", function(result){
					 	$.ajaxSetup({  
					 	    async : false,
					 	    cache : false
					 	}); 
					 	...
					 	for (var i=0 ;i< lst.length;i++){
					 		var sid = $(lst[i]).attr("sysid");
					 		var snme = $(lst[i]).attr("sysname");
					 		if(sid!=null){
					 			var url = "menu.action?subSysId=" + sid + "&subSysName=" + snme;
					 			 
					 			$.get(url,__bind(function(result){
					 	...
					 	//var alst = $("ul.nav li a");
					 	var url = $("#firstUrl").attr("href");
						$("#cbody").attr("src",url);
						//alert(url);
						$("ul.nav").superfish().find('ul').bgIframe();
