...
import fmss.common.db.achieveTimeFromDB;
...
public class UBaseSysLogService extends CommonService {
	...
	public synchronized void saveUBaseSysLog(UBaseSysLogDO sysLog) {
		try {
			...
			System.out.println(sysLog.getExecTime()+"kkkkkkkkkkkkkkkkkkkkkkkkkkkkk");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public synchronized void saveUBaseSysLog(LoginDO loginUser, String logType,
		...
		try {
			...
			System.out.println(sysLog.getExecTime()+"ddddddddddddddddddddddddd");
			// 保存日志信息
			this.save(sysLog);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public synchronized UBaseSysLogDO setUBaseSysLog(LoginDO loginUser,
		...
		/*achieveTimeFromDB atfd=new achieveTimeFromDB();
		
		String ddd=atfd.atfd();

		sysLog.setExecTime(ts1);
		
		System.out.println(ts1+"iiiiiiiiiiiiiiiiiiiiii");*/
		
		Timestamp ts1 = new Timestamp(System.currentTimeMillis());
		
		ts1 = Timestamp.valueOf(ddd);
