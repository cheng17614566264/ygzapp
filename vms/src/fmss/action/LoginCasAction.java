...
public class LoginCasAction extends BaseAction {
	...
	public String execute() throws Exception {
		...
		if (object != null) {
				...
				if(StringUtils.isNotEmpty(userId)){ 
					...
					if(usdo==null){
						session.setAttribute("ERROR_CODE", "99999");
						session.setAttribute("ERROR_MSG", "系统不存在该用户ID");
					    return INPUT;
					}else{
						...
						session.setAttribute("ERROR_CODE", "99999");
						session.setAttribute("ERROR_MSG", "单点登录后，获取用户ID失败!");
						log.info("单点登录后，获取用户ID失败!");
						this.setMess("单点登录后，获取用户ID失败");
						return INPUT;
				  }
 			}else{
					session.setAttribute("ERROR_CODE", "99999");
...
//		throw new java.lang.RuntimeException(mess);
 	}
