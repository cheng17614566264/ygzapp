...
public class HolidayTypeBaseAuditBase extends AuditBase {
	...
	public void auditThis(Object o) {
		HolidayTypeChangeDO info = (HolidayTypeChangeDO) o;
		dao.update("update " + HTTBAK_TABLE
	...
	public void onSave(Object o) {
		Assert.isTrue(getColumnFields().length == getValueFields().length);
		HolidayTypeChangeDO holiday = (HolidayTypeChangeDO) o;
		String sql2 = "select count(*) from " + HTTBAK_TABLE
		+" where holiday_name=? " ;
			//+"and audit_status="+AUDIT_STATUS_NOADUITED;
		int count2 = dao.findForInt(sql2, new Object[] {holiday.getHolidayName()});
		if(count2>0) {
			String sql3 ="delete from "+ HTTBAK_TABLE+" where holiday_name=?";
			dao.update(sql3,  new Object[] {holiday.getHolidayName()});
		}
		dao.save(HTTBAK_TABLE, o, getValueFields(), getColumnFields());
	}
}