...
import fmss.dao.entity.VmsCostCenterInst;
...
public class InstAction extends JSONProviderAction{
	...
	private VmsCostCenterInst vei; //2018-07-26新增
	private List leftSel;
	private List rightSel;
	/*private String[] selectedTransTypeRight;
	private String[] selectedTransTypeLeft;*/
	private String[] userList;
	protected static VmsCostCenterInst vv = new VmsCostCenterInst();
	private String instIdDel;
	
	public String getInstIdDel() {
		return instIdDel;
	}

	public void setInstIdDel(String instIdDel) {
		this.instIdDel = instIdDel;
	}
	...
	public VmsCostCenterInst getVei() {
		return vei;
	}

	public void setVei(VmsCostCenterInst vei) {
		this.vei = vei;
	}
...
//							this.sysLogService.saveUBaseSysLog(sysLog);
	...
	*/
	...
	//机构关联配置
	public String instDeploy(){
		String instId = request.getParameter("instId");
		vv.setInstId(instId);
		System.out.println(vv.getInstId()+"://///////");
		if(instId == null||"".equals(instId)){
			vei = new VmsCostCenterInst("0",instId);
			return ERROR;
		}
		String hql = "select u.instName from UBaseInstDO u where u.instId = '"+ instId +"'";
		System.out.println(hql);
		List listName = instService.find(hql);
		String instName = null;
		if(listName != null && listName.size() > 0){
			instName = listName.get(0).toString();
			System.out.println(instName);
		}
		
		vei = new VmsCostCenterInst(instName,instId);
		/*String leftSql = "select v from VmsCostCenterInst v where v.instId = '"+ instId +"' and v.datastatus = '0'";*/
		String leftSql = "select v from VmsCostCenterInst v where v.instId = '"+ instId +"'";
		leftSel = instService.find(leftSql);
		
		/*if(leftSel.size() != 0){
			for(int i=0;i<leftSel.size();i++){
				VmsCostCenterInst vcc = (VmsCostCenterInst) leftSel.get(i);
				System.out.println(vcc);
				String leftCostCenter = vcc.getCostCenter();
				//String leftCostCenter = leftSel.get(i).toString();
				String idList = instId+"-"+leftCostCenter;
				System.out.println(leftCostCenter+":"+i);
				System.out.println(idList);
				//vcc.setIdList(idList);
				vcc.setCostCenter(idList);
			}
		}
		String rightSql = "select v from VmsCostCenterInst v where v.instId = '"+ instId +"' and v.datastatus = '1'"; 
		rightSel = instService.find(rightSql);
		if(rightSel.size() != 0){
			for(int j=0;j<rightSel.size();j++){
				VmsCostCenterInst vcci = (VmsCostCenterInst) rightSel.get(j);
				String rightCostCenter = vcci.getCostCenter();
				String idList = instId+"-"+rightCostCenter;
				System.out.println(idList);
				//vcci.setIdList(idList);
				vcci.setCostCenter(idList);
			}
		}*/
		return SUCCESS;
	}
	...
	/*public String updateDatastatus(){
		if (null != selectedTransTypeRight) {
			for (int i = 0; i < selectedTransTypeRight.length; i++) {
				System.out.println(selectedTransTypeRight[i]+":"+i);
				String instId = selectedTransTypeRight[i].split("-")[0];
				String costCenter = selectedTransTypeRight[i].split("-")[1];
				String sql1 = "update VmsCostCenterInst v set v.datastatus = '1' where v.instId = '"+instId+"' and v.costCenter = '"+costCenter+"'";
				System.out.println(sql1+":1");
				instService.executeUpdate(sql1);
			}
		}
		if(null != selectedTransTypeLeft){
			for (int j = 0; j < selectedTransTypeLeft.length; j++) {
				String instId = selectedTransTypeLeft[j].split("-")[0];
				String costCenter = selectedTransTypeLeft[j].split("-")[1];
				String sql2 = "update VmsCostCenterInst v set v.datastatus = '0' where v.instId = '"+instId+"' and v.costCenter = '"+costCenter+"'";
				System.out.println(sql2+":2");
				instService.executeUpdate(sql2);
			}
		}
	...
	//删除机构
	public String deleteInstDeploy(){
		for(int i=0;i<userList.length;i++){
			String costCenter = userList[i];
			String sql1 = "delete from VmsCostCenterInst where costCenter = '"+costCenter+"'";
			instService.executeUpdate(sql1);
		}
		this.setInstIdDel(vv.getInstId());
		System.out.println(instIdDel+":------------");
		/*String instIdList = null;
		if (null != selectedTransTypeRight) {
			for (int i = 0; i < selectedTransTypeRight.length; i++) {
				System.out.println(selectedTransTypeRight[i]+":"+i);
				instIdList = selectedTransTypeRight[i].split("-")[0];
				String costCenter = selectedTransTypeRight[i].split("-")[1];
				VmsCostCenterInst vcc = new VmsCostCenterInst();
				vcc.setInstId(instId);
				vcc.setCostCenter(costCenter);
				instService.delete(vcc);
				String sql1 = "delete from VmsCostCenterInst where instId = '"+instIdList+"' and costCenter = '"+costCenter+"'";
				//String sql1 = "update VmsCostCenterInst v set v.datastatus = '3' where v.instId = '"+instId+"' and v.costCenter = '"+costCenter+"'";
				instService.executeUpdate(sql1);
			}
		}
		if(null != selectedTransTypeLeft){
			for (int j = 0; j < selectedTransTypeLeft.length; j++) {
				instIdList = selectedTransTypeLeft[j].split("-")[0];
				String costCenter = selectedTransTypeLeft[j].split("-")[1];
				String sql2 = "delete from VmsCostCenterInst where instId = '"+instIdList+"' and costCenter = '"+costCenter+"'";
				//String sql2 = "update VmsCostCenterInst v set v.datastatus = '3' where v.instId = '"+instId+"' and v.costCenter = '"+costCenter+"'";
				VmsCostCenterInst vcc = new VmsCostCenterInst();
				vcc.setInstId(instId);
				vcc.setCostCenter(costCenter);
				instService.delete(vcc);
	...
	public String addInst(){
		String instId = request.getParameter("instId");
		this.setInstIdDel(instId);
		//vei.setInstId(instId);
		return SUCCESS;
	}
	public String insertInst(){
		VmsCostCenterInst vi = new VmsCostCenterInst();
		String instId = request.getParameter("instId");
		this.setInstIdDel(instId);
		vi.setInstId(instId);
		
		String hql1 = "select u.instName from UBaseInstDO u where u.instId = '"+ instId +"'";
		List listName = instService.find(hql1);
		String instName = null;
		if(listName != null && listName.size() > 0){
			instName = listName.get(0).toString();
		}
		
		vi.setInstName(instName);
		vi.setCostCenter(request.getParameter("costCenter"));
		vi.setCostCenterName(request.getParameter("costCenterName"));
		vi.setParentInstId(request.getParameter("parentInstId"));
		vi.setDsource(request.getParameter("dsource"));
		vi.setInstLevel(request.getParameter("instLevel"));
		
		String hql = "select v from VmsCostCenterInst v where v.costCenter = '"+ vi.getCostCenter() +"'";
		rightSel = instService.find(hql);
		if(rightSel.size() > 0){
			return ERROR;
		}
		instService.save(vi);
