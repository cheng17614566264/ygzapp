...
public class HtmlUtil
{
                                      ...
                                      int i)
    {
        ...
        int end = body.indexOf("/>", i); //加入对于XHTML的支持 oofrank@2006.3.21
        if (end==-1){  
        	end = body.indexOf('>', i);
        }
        if (end == -1)
        ...
        result.append(FormHtmlEncoder.encode(value)).append('"').append(' ');  //oofrank add .append(' ') 
        //oofrank 由于两次提交Bug修改之
    ...
    public static String addAttribute(String body, String name, int i)
    {
        ...
        StringBuffer result = new StringBuffer(body.length());
        result.append(body.substring(i, end)).append(' ');
        result.append(name).append(' ').append(body.substring(end));
     ...
     * @param id
     * @param name
     * @param type:text|hidden|checkbox|button 
     * @return
     */
    public static String buildHtmlInput(String id,String name,String type){
    	return "<input id=\""+id+"\" name=\""+name+"\" type=\""+type+"\" />";
    }
    
    public static String buildBR(){
    	return "</br>";
    }
    
    private static int indexOfIgnoreCase(String src, String subS, int startIndex) {
